...
SymbolTableRow::SymbolTableRow(int off, string sec, bool isLocal, bool isDefined, bool ext) : offset(off), section(sec), local(isLocal), defined(isDefined), extern_sym(ext) {}
...
LinkerSectionTable::LinkerSectionTable(int sz, string section_name, int section_id, int start_address) : SectionTable(sz, section_name, section_id), start_address(start_address) {}